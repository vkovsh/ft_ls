...
void            get_permission_str(t_catalog *c,
                char *perm)
{
    ...
    i = 0;
    while (++i <= PERM_TOTAL)
        perm[i] = (c->clstat.st_mode & pm[i]) ? PERM_FULL[i] : '-';
}
...
void			print_verbose_info(t_catalog *catalog)
{
    ...
    if (catalog->filetype == LNK_FILE)
	{
        ...
        print_from_stat(catalog, target_name, cp[catalog->filetype]);
	}
	else
		print_from_stat(catalog, NULL, cp[catalog->filetype]);
}